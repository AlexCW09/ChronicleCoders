...
func decision() {
    if stayInOrLeaveRoom() {
        print("\n     \(gender) \(name) decided to go to the library to get more reading in, however they came across a suspicious activity while walking to the library containing info about kingdom and magic. While observing, they noticed ripped pages in one of the guards hands talking to a familiar figure yelling at them. Suddenly, one of the guards looked at \(name)'s direction causing \(name) to run back to his room. \(name) was struggling to sleep due to them questioning what they could have possibly have seen")
    }
    else {
        print("\(gender) \(name) decided to stay in their bed and slept through the night")
    }
}
