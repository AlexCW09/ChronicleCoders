...
func chapterTwo()
{
    ...
    {
        print("As the old man is distracted, \(mainCharacter.name) had to decide whether try to kill the old man or try to apprehend him. (y for kill, n for apprehend)")
        if let userDecision = readLine()
        {
            if userDecision == "y"
            {
    ...
    {
        print("Read the exciting conclusion to chapter 2? (y/n)")
        if let userDecision = readLine()
        {
            if userDecision == "y"
            {
                //Read conclusion to chapter 2
                showStory(story: story.conclusion)
